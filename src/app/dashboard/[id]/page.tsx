...
function StatusTag({ status }: { status: string }) {
  ...
  const styles: Record<string, string> = {
    ongoing: 'bg-accent-ongoing text-white',
    built_upon: 'bg-accent-built text-white',
    dropped: 'bg-slate-500 text-white',
    reframed: 'bg-primary text-white',
  };
  const cls = styles[s] ?? 'bg-primary text-white';
  return (
    ...
    <div className="min-h-screen bg-surface">
      <header className="p-4 border-b border-border bg-surface-elevated backdrop-blur">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <h1 className="text-xl font-bold text-heading">Shine Reflection Report</h1>
          <div className="flex gap-4">
            <Link href="/dashboard" className="text-muted hover:text-heading font-medium text-sm">
              ← Dashboard
            </Link>
      ...
      <main className="max-w-4xl mx-auto p-6 space-y-8">
        <p className="text-muted text-sm">Session: {code} · {new Date(meeting.created_at).toLocaleString()}</p>

        {/* 1. Voices to Revisit */}
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Voices to Revisit</h2>
          {voices.length === 0 ? (
            <p className="text-muted text-sm">No items.</p>
          ) : (
            <div className="space-y-4">
              {voices.map((v, i) => (
                <div key={i} className="p-4 rounded-lg bg-surface-subtle border border-border">
                  <h3 className="font-medium text-heading">{v.idea ?? '—'}</h3>
                  {v.introduced_by && (
                    <p className="text-sm text-heading mt-1 font-medium">
                      Introduced by {v.introduced_by}
                    </p>
                  )}
                  <p className="text-sm text-heading mt-2">{v.what_happened ?? ''}</p>
                  {Array.isArray(v.suggested_phrases) && v.suggested_phrases.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {v.suggested_phrases.map((phrase, j) => (
                        <span key={j} className="flex items-center gap-1">
                          <span className="px-2 py-1 rounded bg-primary/10 text-heading text-sm italic">&ldquo;{phrase}&rdquo;</span>
        ...
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Idea Flow & Ownership</h2>
          {ideaFlow.length === 0 ? (
            <p className="text-muted text-sm">No items.</p>
          ) : (
            <ul className="space-y-2">
              {ideaFlow.map((item, i) => (
                <li key={i} className="flex justify-between items-start p-2 rounded bg-surface-subtle">
                  <div>
                    <span className="font-medium text-heading">{item.idea ?? '—'}</span>
                    {item.introduced_by && (
                      <span className="text-heading text-sm ml-1 font-medium">
        ...
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Recognition & Amplification Patterns</h2>
          <div className="space-y-3">
            {Array.isArray(recog.explicit_credit_examples) && recog.explicit_credit_examples.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-heading">Explicit credit examples</h4>
            ...
            {Array.isArray(recog.missed_credit_opportunities) && recog.missed_credit_opportunities.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-heading">Opportunities to amplify recognition</h4>
                <ul className="list-disc list-inside text-sm text-heading">
            ...
            {recog.overall_note && <p className="text-sm text-heading italic">{recog.overall_note}</p>}
          </div>
        </section>

        {/* 4. Speaking Overlap Insights */}
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Speaking Overlap Insights</h2>
          ...
          <div className="space-y-3">
            ...
            {Array.isArray(interrupt.patterns) && interrupt.patterns.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-heading">Patterns</h4>
                <ul className="list-disc list-inside text-sm text-heading">
            ...
            {Array.isArray(interrupt.facilitation_suggestions) && interrupt.facilitation_suggestions.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-heading">Facilitation suggestions</h4>
                <ul className="list-disc list-inside text-sm text-heading">
            ...
            {(!interrupt.patterns?.length && !interrupt.facilitation_suggestions?.length && (interrupt.total_overlap_events ?? 0) === 0) && (
              <p className="text-muted text-sm">No overlapping speech detected in this meeting.</p>
            )}
          </div>
        </section>
        ...
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Participation Notes</h2>
          <div className="space-y-3">
            {Array.isArray(part.observations) && part.observations.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-heading">Observations</h4>
                <ul className="list-disc list-inside text-sm text-heading">
            ...
            {Array.isArray(part.facilitation_prompts) && part.facilitation_prompts.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-heading">Facilitation prompts</h4>
                <ul className="list-disc list-inside text-sm text-heading">
        ...
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Try This Next Time</h2>
          {tryNext.length === 0 ? (
            <p className="text-muted text-sm">No suggestions.</p>
          ) : (
        ...
        <section className="bg-white/80 rounded-xl border border-border p-6">
          <h2 className="text-lg font-semibold text-heading mb-4">Amplification Toolkit</h2>
          {toolkit.length === 0 ? (
            <p className="text-muted text-sm">No phrases.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {toolkit.map((phrase, i) => (
                <span key={i} className="flex items-center gap-1">
                  <span className="px-3 py-2 rounded-lg bg-primary/10 text-heading text-sm italic">&ldquo;{phrase}&rdquo;</span>
                  <CopyButton text={phrase} />
                </span>
              ))}
            </div>
